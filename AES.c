...
// print a key (or any smallish buffer) onto screen as hex
...
static void mix_col(byte *state) {
	...
	byte a2 = state[2];
	byte a3 = state[3];
	state[0] = lookup_g2[a0] ^ lookup_g3[a1] ^ a2 ^ a3;  //appropriately xor bytes per algorithm to obtain new byte values
	state[1] = lookup_g2[a1] ^ lookup_g3[a2] ^ a3 ^ a0;
	state[2] = lookup_g2[a2] ^ lookup_g3[a3] ^ a0 ^ a1;
	state[3] = lookup_g2[a3] ^ lookup_g3[a0] ^ a1 ^ a2;
	/*for (int i = 0; i < 4; i++)
	{*/
		printf("%02x%s%02x%s%02x%s%02x", state[i], " ", state[i+1], " ", state[i+2], " ", state[i+3]); //Print each column mix to display
	    //printf("%02x%s", state[i], "\n");															   //intermediate key values
	//}
	printf("%s", "\n");
}
...
int main(void) {
	...
	byte msg[] = { 0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10 };
	byte encrypted[16], decrypted[16];

	printf("AES\r\n\n");
	Pretty(key, 16, "Key:         ");
	Pretty(msg, 16, "Plaintext:   ");
	printf("%s", "Printing intermediate keys in 4 byte rows...\n");
